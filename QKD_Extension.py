...
def simulation_COW():
    ...
    # Alice
    ls_params = {"frequency": 80e6, "mean_photon_num": 0.1}
    alice = QKDNode("alice", tl, encoding=time_bin, stack_size=1, qkdtype=2)
    alice.set_seed(0)
    ...
    detector_params = [{"efficiency": 0.8, "dark_count": 10, "time_resolution": 10, "count_rate": 50e6},
                       {"efficiency": 0.8, "dark_count": 10, "time_resolution": 10, "count_rate": 50e6}]
    bob = QKDNode("bob", tl, encoding=time_bin, stack_size=1, qkdtype=2)
    bob.set_seed(1)
...
def plot_graph(d_step, d_lim, att_lim, keysize):
    ...
    while d <= d_lim:
        ...
        skr_list_cow.append(SECRET_KEY_RATE)
        qber_list_cow.append(np.mean(QBER))
        print()
        print(str((d/d_lim)*100)+'% concluído')
        d += d_step
    skr.append(skr_list_cow)
    qber.append(qber_list_cow)
